import {
  Fragment,
  computed,
  createBaseVNode,
  createBlock,
  createCommentVNode,
  createElementBlock,
  createTextVNode,
  createVNode,
  defineComponent,
  getCurrentInstance,
  inject,
  isRef,
  nextTick,
  normalizeClass,
  normalizeStyle,
  onMounted,
  onUpdated,
  openBlock,
  popScopeId,
  provide,
  pushScopeId,
  reactive,
  ref,
  renderList,
  renderSlot,
  toDisplayString,
  toRaw,
  unref,
  vModelText,
  vShow,
  watch,
  withDirectives,
  withKeys,
  withModifiers
} from "./chunk-J6475X5X.js";

// node_modules/@neshan-maps-platform/vue3-openlayers/dist/Vue3-Openlayers.js
var Fe = "data:image/svg+xml;base64,";
var ve = "data:image/png;base64,";
var Me = "data:image/png;base64,";
var oe = (e) => (pushScopeId("data-v-53883187"), e = e(), popScopeId(), e);
var Je = { class: "justify-between d-flex" };
var Ne = ["onClick", "selected"];
var Ze = ["src"];
var We = { class: "text-sm" };
var Xe = { class: "d-flex justify-around poi-traffic" };
var Ke = ["selected"];
var Ue = oe(() => createBaseVNode("img", { src: ve }, null, -1));
var qe = oe(() => createBaseVNode("div", { class: "text-sm" }, "Poi", -1));
var _e = [
  Ue,
  qe
];
var $e = ["selected"];
var et = oe(() => createBaseVNode("img", { src: Me }, null, -1));
var tt = oe(() => createBaseVNode("div", { class: "text-sm" }, "Traffic", -1));
var st = [
  et,
  tt
];
var at = defineComponent({
  __name: "DesktopLayers",
  props: {
    tiles: {
      type: Array,
      default: () => []
    },
    layersClass: [String, Array, Object],
    layersStyle: Object
  },
  setup(e) {
    const t = inject("store");
    return (r, s) => (openBlock(), createElementBlock("div", {
      style: normalizeStyle(e.layersStyle),
      class: normalizeClass(["map-settings o-hidden pos-absolute", e.layersClass])
    }, [
      createBaseVNode("div", Je, [
        (openBlock(true), createElementBlock(Fragment, null, renderList(e.tiles, (a) => (openBlock(), createElementBlock("div", {
          key: a.title,
          onClick: (o) => unref(t).actions.map.setMapType(a.title),
          class: "pointer tile",
          selected: a.title == unref(t).state.mapType
        }, [
          createBaseVNode("img", {
            src: a.url
          }, null, 8, Ze),
          createBaseVNode("div", We, toDisplayString(a.title[0].toUpperCase() + a.title.slice(1)), 1)
        ], 8, Ne))), 128))
      ]),
      createBaseVNode("div", Xe, [
        createBaseVNode("div", {
          class: "pointer tile",
          selected: unref(t).state.poiLayer,
          onClick: s[0] || (s[0] = (a) => unref(t).actions.map.togglePoi())
        }, _e, 8, Ke),
        createBaseVNode("div", {
          class: "pointer tile",
          selected: unref(t).state.trafficLayer,
          onClick: s[1] || (s[1] = (a) => unref(t).actions.map.toggleTraffic())
        }, st, 8, $e)
      ])
    ], 6));
  }
});
var j = (e, t) => {
  const r = e.__vccOpts || e;
  for (const [s, a] of t)
    r[s] = a;
  return r;
};
var rt = j(at, [["__scopeId", "data-v-53883187"]]);
var ne = (e) => (pushScopeId("data-v-8a56aabe"), e = e(), popScopeId(), e);
var ot = ["activated"];
var nt = { class: "map-layers d-flex justify-center" };
var it = ["selected"];
var lt = ne(() => createBaseVNode("img", { src: ve }, null, -1));
var ct = ne(() => createBaseVNode("div", { class: "desc text-sm" }, "POI", -1));
var At = [
  lt,
  ct
];
var ut = ["selected"];
var dt = ne(() => createBaseVNode("img", { src: Me }, null, -1));
var gt = ne(() => createBaseVNode("div", { class: "desc text-sm" }, "Traffic", -1));
var mt = [
  dt,
  gt
];
var wt = { class: "map-types d-flex justify-center" };
var yt = ["selected", "onClick"];
var Ct = ["src"];
var pt = { class: "desc text-sm" };
var ht = defineComponent({
  __name: "MobileLayers",
  props: {
    modelValue: Boolean,
    tiles: {
      type: Array,
      default: () => []
    },
    layersClass: [String, Array, Object],
    layersStyle: Object
  },
  emits: ["update:modelValue"],
  setup(e, { emit: t }) {
    const r = e, s = getCurrentInstance(), a = inject("store"), o = (A) => {
      t("update:modelValue", A);
    }, l = ref(false);
    watch(
      () => r.modelValue,
      (A) => {
        A && setTimeout(() => {
          var u;
          l.value = true, (u = s == null ? void 0 : s.proxy) == null || u.$forceUpdate();
        }, 200);
      }
    );
    const c = () => {
      var A;
      l.value = false, (A = s == null ? void 0 : s.proxy) == null || A.$forceUpdate(), setTimeout(() => {
        o(false);
      }, 200);
    };
    return (A, u) => withDirectives((openBlock(), createElementBlock("div", {
      class: normalizeClass(["mobile-settings-container pos-absolute o-hidden", e.layersClass]),
      style: normalizeStyle(e.layersStyle),
      onClick: c
    }, [
      createBaseVNode("div", {
        class: "mobile-settings pos-absolute px-1",
        activated: l.value,
        onClick: u[2] || (u[2] = withModifiers(() => {
        }, ["prevent", "stop"]))
      }, [
        createBaseVNode("div", nt, [
          createBaseVNode("div", {
            class: "d-flex justify-between align-center tile",
            selected: unref(a).state.poiLayer,
            onClick: u[0] || (u[0] = (i) => unref(a).actions.map.togglePoi())
          }, At, 8, it),
          createBaseVNode("div", {
            class: "d-flex justify-between align-center tile",
            selected: unref(a).state.trafficLayer,
            onClick: u[1] || (u[1] = (i) => unref(a).actions.map.toggleTraffic())
          }, mt, 8, ut)
        ]),
        createBaseVNode("div", wt, [
          (openBlock(true), createElementBlock(Fragment, null, renderList(e.tiles, (i) => (openBlock(), createElementBlock("div", {
            key: i.title,
            selected: i.title == unref(a).state.mapType,
            class: "d-flex justify-between align-center tile",
            onClick: (m) => unref(a).actions.map.setMapType(i.title)
          }, [
            createBaseVNode("img", {
              src: i.url
            }, null, 8, Ct),
            createBaseVNode("div", pt, toDisplayString(i.title[0].toUpperCase() + i.title.slice(1)), 1)
          ], 8, yt))), 128))
        ])
      ], 8, ot)
    ], 6)), [
      [vShow, e.modelValue]
    ]);
  }
});
var vt = j(ht, [["__scopeId", "data-v-8a56aabe"]]);
var q = {
  close: {
    d: "M19,6.41L17.59,5L12,10.59L6.41,5L5,6.41L10.59,12L5,17.59L6.41,19L12,13.41L17.59,19L19,17.59L13.41,12L19,6.41Z"
  },
  magnet: {
    d: "M9.5,3A6.5,6.5 0 0,1 16,9.5C16,11.11 15.41,12.59 14.44,13.73L14.71,14H15.5L20.5,19L19,20.5L14,15.5V14.71L13.73,14.44C12.59,15.41 11.11,16 9.5,16A6.5,6.5 0 0,1 3,9.5A6.5,6.5 0 0,1 9.5,3M9.5,5C7,5 5,7 5,9.5C5,12 7,14 9.5,14C12,14 14,12 14,9.5C14,7 12,5 9.5,5Z"
  },
  neighborhood_landmark: {
    d: "M17,16H15V22H12V17H8V22H5V16H3L10,10L17,16M6,2L10,6H9V9H7V6H5V9H3V6H2L6,2M18,3L23,8H22V12H19V9H17V12H15.34L14,10.87V8H13L18,3Z"
  },
  formal_school: {
    d: "M12,3L1,9L12,15L21,10.09V17H23V9M5,13.18V17.18L12,21L19,17.18V13.18L12,17L5,13.18Z"
  },
  university: {
    d: "M12,3L1,9L12,15L21,10.09V17H23V9M5,13.18V17.18L12,21L19,17.18V13.18L12,17L5,13.18Z"
  },
  primary: {
    d: "M18.1,4.8C18,4.3 17.6,4 17.1,4H13L13.2,7H10.8L11,4H6.8C6.3,4 5.9,4.4 5.8,4.8L3.1,18.8C3,19.4 3.5,20 4.1,20H10L10.3,15H13.7L14,20H19.8C20.4,20 20.9,19.4 20.8,18.8L18.1,4.8M10.4,13L10.6,9H13.2L13.4,13H10.4Z"
  },
  secondary: {
    d: "M18.1,4.8C18,4.3 17.6,4 17.1,4H13L13.2,7H10.8L11,4H6.8C6.3,4 5.9,4.4 5.8,4.8L3.1,18.8C3,19.4 3.5,20 4.1,20H10L10.3,15H13.7L14,20H19.8C20.4,20 20.9,19.4 20.8,18.8L18.1,4.8M10.4,13L10.6,9H13.2L13.4,13H10.4Z",
    scale: 0.8
  },
  tertiary: {
    d: "M18.1,4.8C18,4.3 17.6,4 17.1,4H13L13.2,7H10.8L11,4H6.8C6.3,4 5.9,4.4 5.8,4.8L3.1,18.8C3,19.4 3.5,20 4.1,20H10L10.3,15H13.7L14,20H19.8C20.4,20 20.9,19.4 20.8,18.8L18.1,4.8M10.4,13L10.6,9H13.2L13.4,13H10.4Z",
    scale: 0.6
  },
  vaccine: {
    d: "M11.15,15.18L9.73,13.77L11.15,12.35L12.56,13.77L13.97,12.35L12.56,10.94L13.97,9.53L15.39,10.94L16.8,9.53L13.97,6.7L6.9,13.77L9.73,16.6L11.15,15.18M3.08,19L6.2,15.89L4.08,13.77L13.97,3.87L16.1,6L17.5,4.58L16.1,3.16L17.5,1.75L21.75,6L20.34,7.4L18.92,6L17.5,7.4L19.63,9.53L9.73,19.42L7.61,17.3L3.08,21.84V19Z"
  },
  subway_station: {
    d: "M8.5,15A1,1 0 0,1 9.5,16A1,1 0 0,1 8.5,17A1,1 0 0,1 7.5,16A1,1 0 0,1 8.5,15M7,9H17V14H7V9M15.5,15A1,1 0 0,1 16.5,16A1,1 0 0,1 15.5,17A1,1 0 0,1 14.5,16A1,1 0 0,1 15.5,15M18,15.88V9C18,6.38 15.32,6 12,6C9,6 6,6.37 6,9V15.88A2.62,2.62 0 0,0 8.62,18.5L7.5,19.62V20H9.17L10.67,18.5H13.5L15,20H16.5V19.62L15.37,18.5C16.82,18.5 18,17.33 18,15.88M17.8,2.8C20.47,3.84 22,6.05 22,8.86V22H2V8.86C2,6.05 3.53,3.84 6.2,2.8C8,2.09 10.14,2 12,2C13.86,2 16,2.09 17.8,2.8Z"
  },
  metro_entrance: {
    d: "M6 11V13H4V11H6M22 5H17V19H22V5M7 5H2L2 19H7V5M22 3C23.11 3 24 3.89 24 5V21H0V5C0 3.89 .894 3 2 3H9V19H15V3H22M20 11H18V13H20V11Z",
    scale: 0.8
  },
  footway: {
    d: "M10.74,11.72C11.21,12.95 11.16,14.23 9.75,14.74C6.85,15.81 6.2,13 6.16,12.86L10.74,11.72M5.71,10.91L10.03,9.84C9.84,8.79 10.13,7.74 10.13,6.5C10.13,4.82 8.8,1.53 6.68,2.06C4.26,2.66 3.91,5.35 4,6.65C4.12,7.95 5.64,10.73 5.71,10.91M17.85,19.85C17.82,20 17.16,22.8 14.26,21.74C12.86,21.22 12.8,19.94 13.27,18.71L17.85,19.85M20,13.65C20.1,12.35 19.76,9.65 17.33,9.05C15.22,8.5 13.89,11.81 13.89,13.5C13.89,14.73 14.17,15.78 14,16.83L18.3,17.9C18.38,17.72 19.89,14.94 20,13.65Z"
  },
  department_store: {
    d: "M12,18H6V14H12M21,14V12L20,7H4L3,12V14H4V20H14V14H18V20H20V14M20,4H4V6H20V4Z"
  },
  internet_cafe: {
    d: "M8.95 13.4H6.58A5.5 5.5 0 0 1 6.58 10.6H8.95A11.56 11.56 0 0 0 8.85 12A11.56 11.56 0 0 0 8.95 13.4M7.16 9.2H9.2A12.06 12.06 0 0 1 10.18 6.71A5.55 5.55 0 0 0 7.16 9.2M16.84 9.2A5.59 5.59 0 0 0 13.81 6.71A10.95 10.95 0 0 1 14.78 9.2M12 17.57A9.5 9.5 0 0 0 13.34 14.8H10.66A9.5 9.5 0 0 0 12 17.57M12 6.42A9.53 9.53 0 0 0 10.66 9.2H13.34A9.53 9.53 0 0 0 12 6.42M7.16 14.8A5.61 5.61 0 0 0 10.18 17.29A12.06 12.06 0 0 1 9.2 14.8M21 5V19A2 2 0 0 1 19 21H5A2 2 0 0 1 3 19V5A2 2 0 0 1 5 3H19A2 2 0 0 1 21 5M19 12A7 7 0 1 0 12 19A7 7 0 0 0 19 12M15.15 12A11.56 11.56 0 0 1 15.05 13.4H17.42A5.5 5.5 0 0 0 17.42 10.6H15.05A11.56 11.56 0 0 1 15.15 12M13.81 17.29A5.62 5.62 0 0 0 16.84 14.8H14.78A10.95 10.95 0 0 1 13.81 17.29M10.36 10.6A8.81 8.81 0 0 0 10.36 13.4H13.64A10.3 10.3 0 0 0 13.75 12A10.21 10.21 0 0 0 13.64 10.6Z"
  },
  park: {
    d: "M10,21V18H3L8,13H5L10,8H7L12,3L17,8H14L19,13H16L21,18H14V21H10Z"
  },
  hotel: {
    d: "M480 0C497.7 0 512 14.33 512 32C512 49.67 497.7 64 480 64V448C497.7 448 512 462.3 512 480C512 497.7 497.7 512 480 512H304V448H208V512H32C14.33 512 0 497.7 0 480C0 462.3 14.33 448 32 448V64C14.33 64 0 49.67 0 32C0 14.33 14.33 0 32 0H480zM112 96C103.2 96 96 103.2 96 112V144C96 152.8 103.2 160 112 160H144C152.8 160 160 152.8 160 144V112C160 103.2 152.8 96 144 96H112zM224 144C224 152.8 231.2 160 240 160H272C280.8 160 288 152.8 288 144V112C288 103.2 280.8 96 272 96H240C231.2 96 224 103.2 224 112V144zM368 96C359.2 96 352 103.2 352 112V144C352 152.8 359.2 160 368 160H400C408.8 160 416 152.8 416 144V112C416 103.2 408.8 96 400 96H368zM96 240C96 248.8 103.2 256 112 256H144C152.8 256 160 248.8 160 240V208C160 199.2 152.8 192 144 192H112C103.2 192 96 199.2 96 208V240zM240 192C231.2 192 224 199.2 224 208V240C224 248.8 231.2 256 240 256H272C280.8 256 288 248.8 288 240V208C288 199.2 280.8 192 272 192H240zM352 240C352 248.8 359.2 256 368 256H400C408.8 256 416 248.8 416 240V208C416 199.2 408.8 192 400 192H368C359.2 192 352 199.2 352 208V240zM256 288C211.2 288 173.5 318.7 162.1 360.2C159.7 373.1 170.7 384 184 384H328C341.3 384 352.3 373.1 349 360.2C338.5 318.7 300.8 288 256 288z"
  },
  local_government_office: {
    d: "M5,3V21H11V17.5H13V21H19V3H5M7,5H9V7H7V5M11,5H13V7H11V5M15,5H17V7H15V5M7,9H9V11H7V9M11,9H13V11H11V9M15,9H17V11H15V9M7,13H9V15H7V13M11,13H13V15H11V13M15,13H17V15H15V13M7,17H9V19H7V17M15,17H17V19H15V17Z"
  },
  marker: {
    d: "M12,11.5A2.5,2.5 0 0,1 9.5,9A2.5,2.5 0 0,1 12,6.5A2.5,2.5 0 0,1 14.5,9A2.5,2.5 0 0,1 12,11.5M12,2A7,7 0 0,0 5,9C5,14.25 12,22 12,22C12,22 19,14.25 19,9A7,7 0 0,0 12,2Z"
  },
  city: {
    d: "M19,15H17V13H19M19,19H17V17H19M13,7H11V5H13M13,11H11V9H13M13,15H11V13H13M13,19H11V17H13M7,11H5V9H7M7,15H5V13H7M7,19H5V17H7M15,11V5L12,2L9,5V7H3V21H21V11H15Z"
  },
  residential: {
    d: "M17,16H15V22H12V17H8V22H5V16H3L10,10L17,16M6,2L10,6H9V9H7V6H5V9H3V6H2L6,2M18,3L23,8H22V12H19V9H17V12H15.34L14,10.87V8H13L18,3Z"
  },
  back: {
    d: "M20,11V13H8L13.5,18.5L12.08,19.92L4.16,12L12.08,4.08L13.5,5.5L8,11H20Z"
  }
};
q.neighborhood_landmark.d, q.primary.d;
var Mt = ["fill", "d"];
var T = defineComponent({
  __name: "IconComponent",
  props: {
    name: {
      type: String,
      default: "close"
    },
    size: {
      type: Number,
      default: 15
    },
    color: String,
    scale: {
      type: Boolean,
      default: true
    }
  },
  setup(e) {
    const t = e, r = computed(() => t.scale ? s.state.scale * t.size : t.size), s = inject("store");
    return (a, o) => (openBlock(), createElementBlock("svg", {
      style: normalizeStyle(`width: ${unref(r)}px; height: ${unref(r)}px`),
      viewBox: "0 0 24 24"
    }, [
      createBaseVNode("path", {
        fill: e.color || "currentColor",
        d: unref(q)[e.name].d
      }, null, 8, Mt)
    ], 4));
  }
});
var kt = "data:image/png;base64,";
var le = {
  reverse: "https://api.neshan.org/v4/reverse",
  search: "https://api.neshan.org/v1/search",
  map: "https://static.neshan.org/sdk/openlayers/5.3.0/ol.js"
};
var W = {
  main: kt,
  search: "https://static.neshanmap.ir/poi/square/",
  colorSvg: "https://static.neshanmap.ir/poi/svg/color/",
  greySvg: "https://static.neshanmap.ir/poi/svg/grey/",
  preview256: "https://static.neshanmap.ir/poi/256/",
  searchPrev: "/images/search-marker-prev.png"
};
var Ce = [
  {
    title: "osm-bright",
    url: "https://static.neshan.org/sdk/examples/maptypes/images/map-types-bright.png"
  },
  {
    title: "standard-night",
    url: "https://static.neshan.org/sdk/examples/maptypes/images/map-types-standard-night.png"
  },
  {
    title: "standard-day",
    url: "https://static.neshan.org/sdk/examples/maptypes/images/map-types-standard-day.png"
  },
  {
    title: "neshan",
    url: "https://static.neshan.org/sdk/examples/maptypes/images/map-types-neshan.png"
  },
  {
    title: "dreamy-gold",
    url: "https://static.neshan.org/sdk/examples/maptypes/images/map-types-dreamy-gold.png"
  },
  {
    title: "dreamy",
    url: "https://static.neshan.org/sdk/examples/maptypes/images/map-types-dreamy.png"
  }
];
var se = {
  xs: false,
  sm: false,
  md: false,
  lg: false,
  xl: false
};
var ie = {
  xs: 0,
  sm: 600,
  md: 1024,
  lg: 1440,
  xl: 1920
};
var R = {
  width: "18em",
  bottomSheetNormalHeight: "5em",
  bottomSheetloadingHeight: "2em",
  bottomSheetBorderRadius: "1em 1em 0 0"
};
var Dt = {
  width: "700px",
  height: "1000px"
};
var ae = {
  initialZoom: 12,
  maxZoom: 18.5
};
var Q = {
  high: [0, -60],
  short: [0, -40]
};
var ft = ["activated"];
var Bt = defineComponent({
  __name: "SearchSection",
  props: {
    searchBoxClass: [String, Array, Object],
    searchBoxStyle: Object,
    searchText: {
      type: String
    }
  },
  emits: [
    "update:search-text",
    "update:search-coords",
    "submit"
  ],
  setup(e, { emit: t }) {
    const r = e, s = inject("store"), a = R.width, o = computed({
      get: () => r.searchText,
      set: (u) => {
        t("update:search-text", u), A(u);
      }
    }), l = (u = "") => {
      var m;
      const i = u || ((m = o.value) == null ? void 0 : m.trim());
      t("submit", i);
    };
    let c;
    const A = (u = o.value, i = 1e3) => {
      if (clearTimeout(c), u)
        s.state.searchLoading || (s.state.searchLoading = true);
      else {
        s.state.searchLoading = false;
        return;
      }
      c = window.setTimeout(() => {
        l(u);
      }, i);
    };
    return (u, i) => (openBlock(), createElementBlock("div", {
      activated: unref(s).state.drawerActivation,
      style: normalizeStyle(`width: ${unref(a)}; ${e.searchBoxStyle}`),
      class: normalizeClass(["map-search-box d-flex justify-between align-center mx-auto", e.searchBoxClass])
    }, [
      withDirectives(createBaseVNode("input", {
        type: "search",
        name: "search",
        placeholder: "جستجو",
        "onUpdate:modelValue": i[0] || (i[0] = (m) => isRef(o) ? o.value = m : null),
        onKeydown: i[1] || (i[1] = withKeys((m) => A(unref(o), 0), ["enter"])),
        onFocus: i[2] || (i[2] = (m) => unref(s).state.drawerActivation = true)
      }, null, 544), [
        [vModelText, unref(o)]
      ]),
      unref(s).state.drawerShowDetails && !unref(s).getters.touchPlatform ? (openBlock(), createElementBlock("button", {
        key: 0,
        onClick: i[3] || (i[3] = (m) => unref(s).state.drawerShowDetails = false),
        class: "pointer"
      }, [
        createVNode(T, {
          name: "back",
          size: 19
        })
      ])) : createCommentVNode("", true),
      unref(s).state.drawerActivation ? (openBlock(), createElementBlock("button", {
        key: 2,
        class: "pointer",
        onClick: i[5] || (i[5] = (m) => unref(s).state.drawerActivation = false)
      }, [
        createVNode(T, {
          name: "close",
          size: 19
        })
      ])) : (openBlock(), createElementBlock("button", {
        key: 1,
        onClick: i[4] || (i[4] = (m) => unref(s).state.drawerActivation = true),
        class: "pointer"
      }, [
        createVNode(T, {
          name: "magnet",
          size: 19
        })
      ]))
    ], 14, ft));
  }
});
var bt = j(Bt, [["__scopeId", "data-v-353796a4"]]);
var Lt = (e) => (pushScopeId("data-v-64c8a44d"), e = e(), popScopeId(), e);
var Et = {
  key: 0,
  class: "list-item"
};
var Ht = { class: "text-lg" };
var It = { class: "text-caption" };
var Rt = Lt(() => createBaseVNode("hr", null, null, -1));
var St = defineComponent({
  __name: "ResultItem",
  props: {
    item: Object
  },
  setup(e) {
    return (t, r) => {
      var s, a;
      return openBlock(), createElementBlock("div", null, [
        e.item ? (openBlock(), createElementBlock("div", Et, [
          createBaseVNode("b", Ht, toDisplayString(e.item.title), 1),
          createBaseVNode("div", It, toDisplayString((s = e.item.region) == null ? void 0 : s.split("،").map((o) => " " + o.trim()).reverse().join("،")) + "، " + toDisplayString((a = e.item.address) == null ? void 0 : a.split("-").join("،")) + " " + toDisplayString(e.item.neighbourhood ? `(${e.item.neighbourhood})` : ""), 1)
        ])) : createCommentVNode("", true),
        Rt
      ]);
    };
  }
});
var Ot = j(St, [["__scopeId", "data-v-64c8a44d"]]);
var Gt = (e) => (pushScopeId("data-v-0dc8808b"), e = e(), popScopeId(), e);
var zt = ["dense"];
var Tt = Gt(() => createBaseVNode("div", { class: "loadings" }, null, -1));
var Pt = [
  Tt
];
var Yt = defineComponent({
  __name: "LoadingComp",
  props: {
    color: {
      type: String,
      default: "blue"
    },
    dense: Boolean
  },
  setup(e) {
    return (t, r) => (openBlock(), createElementBlock("div", {
      class: "container",
      dense: e.dense
    }, Pt, 8, zt));
  }
});
var ke = j(Yt, [["__scopeId", "data-v-0dc8808b"]]);
var De = (e) => {
  const t = e.place ? e.place : e.route_name ? e.route_name : "معبر بدون نام";
  return e.neighbourhood ? t + `، ${e.neighbourhood}` : t;
};
var fe = (e) => e ? e instanceof Object ? [e.longitude, e.latitude] : e : void 0;
var jt = async () => {
  try {
    const t = await new Promise(
      (r, s) => {
        navigator.geolocation.getCurrentPosition(
          (a) => {
            r(a);
          },
          (a) => s(a)
        );
      }
    );
    return t && fe(t.coords) || Object.values(xt());
  } catch (e) {
    return console.error(e), {};
  }
};
var xt = () => ({
  longitude: 59.5870851,
  latitude: 36.311559
});
var me = (e, t = "EPSG:3857", r = "EPSG:4326") => ol.proj.transform(e, t, r);
var Qt = (e, t) => {
  const [{ iconScale: r } = {}] = e, s = ts(
    e,
    t == null ? void 0 : t.markersIconCallback,
    {
      isCluster: t == null ? void 0 : t.cluster,
      ...t == null ? void 0 : t.props
    }
  );
  let a, o;
  if (t != null && t.cluster)
    a = $t(s, t);
  else {
    const { styleFunc: l, style: c } = Be({
      hidePopup: t == null ? void 0 : t.hidePopup,
      iconScale: r,
      anchor: t == null ? void 0 : t.anchor
    });
    o = c;
    const A = be(s);
    a = Jt({ style: l, source: A });
  }
  return { layer: a, style: o };
};
var Vt = ({
  iconScale: e = 0.24,
  src: t = W.main,
  anchor: r = [0.5, 0.5]
} = {}) => new ol.style.Icon({
  src: t,
  scale: e,
  anchor: r
});
var Ft = ({ image: e, text: t }) => new ol.style.Style({
  image: e,
  text: t
});
var Be = ({
  hidePopup: e = false,
  iconScale: t,
  anchor: r
}) => {
  const s = Ft({}), a = Xt(s, {
    hidePopup: e,
    iconScale: t,
    anchor: r
  });
  return { style: s, styleFunc: a };
};
var be = (e) => new ol.source.Vector({
  features: e
});
var Jt = ({
  target: e = "points",
  style: t,
  source: r
}) => new ol.layer.Vector({
  target: e,
  source: r,
  style: t
});
var Nt = async (e, t) => await Promise.all(
  e.map(async (r) => {
    const s = [r.location.x, r.location.y], a = me(s, "EPSG:4326", "EPSG:3857"), l = (await Zt(r.type)).url;
    return {
      coords: a,
      text: r.title,
      iconUrl: l,
      iconScale: (t == null ? void 0 : t.iconScale) || 0.25,
      originalItem: r
    };
  })
);
var Zt = async (e) => {
  const t = W.search + e + ".png", r = await Wt(t);
  return { url: r ? t : W.search + "general.png", exists: r };
};
var Wt = (e) => new Promise((t) => {
  fetch(e, { method: "head" }).then((r) => t(r.ok)).catch(() => t(false));
});
var Xt = (e, { hidePopup: t = false, iconScale: r, anchor: s }) => (a) => {
  t && e.getText().setText(a.get("text"));
  const o = a.get("iconProps"), l = {
    src: a.get("iconUrl"),
    iconScale: r,
    anchor: s
  }, c = o ? { ...l, ...o } : l;
  return e.setImage(Vt(c)), e;
};
var Le = (e, { minDistance: t = 30, distance: r = 30 } = {}) => new ol.source.Cluster({
  distance: r,
  minDistance: t,
  source: be(e)
});
var Kt = (e) => new ol.style.Text({
  text: e.toString(),
  font: "bold 15px serif",
  fill: new ol.style.Fill({
    color: [0, 0, 0, 1]
  })
});
var Ut = () => new ol.style.Circle({
  radius: 15,
  stroke: new ol.style.Stroke({
    color: [255, 255, 255, 0.7],
    width: 4
  }),
  fill: new ol.style.Fill({
    color: [0, 153, 255, 0.7]
  })
});
var qt = (e) => new ol.style.Style({
  image: Ut(),
  text: Kt(e)
});
var _t = (e) => {
  const t = {};
  return (r) => {
    const s = r.get("features");
    e || r.set(
      "text",
      s.map((l) => l.get("text"))
    ), r.set("isCluster", s[0].get("isCluster"));
    const a = s.length;
    let o = t[a];
    if (!o)
      if (a !== 1)
        o = qt(a), t[a] = o;
      else {
        const l = s[0].get("iconUrl"), { styleFunc: c } = Be({ iconUrl: l, hidePopup: e });
        o = c(s[0], 100);
      }
    return o;
  };
};
var $t = (e, t) => new ol.layer.Vector({
  source: Le(e, {
    minDistance: t == null ? void 0 : t.clusterDistance,
    distance: t == null ? void 0 : t.clusterDistance
  }),
  style: _t(t == null ? void 0 : t.hidePopup)
});
var Ee = (e) => {
  const t = e.get("features"), r = new ol.extent.createEmpty();
  return t.forEach((s) => {
    ol.extent.extend(r, He(s));
  }), r;
};
var He = (e) => e.getGeometry().getExtent();
var es = (e) => {
  const t = we(e), r = e.getProperties().text;
  return {
    featCoords: t,
    featText: r
  };
};
var we = (e) => e.getGeometry().getCoordinates().slice(0, 2);
var ts = (e, t, r) => e.map((s) => {
  const a = new ol.Feature({
    geometry: new ol.geom.Point(s.coords),
    text: s.text,
    iconUrl: s.iconUrl,
    iconProps: t && t(s),
    mainMarker: s.isReverseMarker,
    ...s.props,
    ...r
  });
  return a.setId(r.id || s.coords.join("-")), a;
});
function pe() {
  let e = false;
  return function(t) {
    (/(android|bb\d+|meego).+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|iris|kindle|lge |maemo|midp|mmp|mobile.+firefox|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|plucker|pocket|psp|series(4|6)0|symbian|treo|up\.(browser|link)|vodafone|wap|windows ce|xda|xiino/i.test(
      t
    ) || /1207|6310|6590|3gso|4thp|50[1-6]i|770s|802s|a wa|abac|ac(er|oo|s\-)|ai(ko|rn)|al(av|ca|co)|amoi|an(ex|ny|yw)|aptu|ar(ch|go)|as(te|us)|attw|au(di|\-m|r |s )|avan|be(ck|ll|nq)|bi(lb|rd)|bl(ac|az)|br(e|v)w|bumb|bw\-(n|u)|c55\/|capi|ccwa|cdm\-|cell|chtm|cldc|cmd\-|co(mp|nd)|craw|da(it|ll|ng)|dbte|dc\-s|devi|dica|dmob|do(c|p)o|ds(12|\-d)|el(49|ai)|em(l2|ul)|er(ic|k0)|esl8|ez([4-7]0|os|wa|ze)|fetc|fly(\-|_)|g1 u|g560|gene|gf\-5|g\-mo|go(\.w|od)|gr(ad|un)|haie|hcit|hd\-(m|p|t)|hei\-|hi(pt|ta)|hp( i|ip)|hs\-c|ht(c(\-| |_|a|g|p|s|t)|tp)|hu(aw|tc)|i\-(20|go|ma)|i230|iac( |\-|\/)|ibro|idea|ig01|ikom|im1k|inno|ipaq|iris|ja(t|v)a|jbro|jemu|jigs|kddi|keji|kgt( |\/)|klon|kpt |kwc\-|kyo(c|k)|le(no|xi)|lg( g|\/(k|l|u)|50|54|\-[a-w])|libw|lynx|m1\-w|m3ga|m50\/|ma(te|ui|xo)|mc(01|21|ca)|m\-cr|me(rc|ri)|mi(o8|oa|ts)|mmef|mo(01|02|bi|de|do|t(\-| |o|v)|zz)|mt(50|p1|v )|mwbp|mywa|n10[0-2]|n20[2-3]|n30(0|2)|n50(0|2|5)|n7(0(0|1)|10)|ne((c|m)\-|on|tf|wf|wg|wt)|nok(6|i)|nzph|o2im|op(ti|wv)|oran|owg1|p800|pan(a|d|t)|pdxg|pg(13|\-([1-8]|c))|phil|pire|pl(ay|uc)|pn\-2|po(ck|rt|se)|prox|psio|pt\-g|qa\-a|qc(07|12|21|32|60|\-[2-7]|i\-)|qtek|r380|r600|raks|rim9|ro(ve|zo)|s55\/|sa(ge|ma|mm|ms|ny|va)|sc(01|h\-|oo|p\-)|sdk\/|se(c(\-|0|1)|47|mc|nd|ri)|sgh\-|shar|sie(\-|m)|sk\-0|sl(45|id)|sm(al|ar|b3|it|t5)|so(ft|ny)|sp(01|h\-|v\-|v )|sy(01|mb)|t2(18|50)|t6(00|10|18)|ta(gt|lk)|tcl\-|tdg\-|tel(i|m)|tim\-|t\-mo|to(pl|sh)|ts(70|m\-|m3|m5)|tx\-9|up(\.b|g1|si)|utst|v400|v750|veri|vi(rg|te)|vk(40|5[0-3]|\-v)|vm40|voda|vulc|vx(52|53|60|61|70|80|81|83|85|98)|w3c(\-| )|webc|whit|wi(g |nc|nw)|wmlb|wonu|x700|yas\-|your|zeto|zte\-/i.test(
      t.substr(0, 4)
    )) && (e = true);
  }(navigator.userAgent || navigator.vendor || window.opera), e;
}
function ss({
  emits: e,
  store: t,
  markerHoverCallback: r,
  zoomOnMarkerClick: s,
  zoomOnResultClick: a,
  popupOnMarkerHover: o,
  popupOnResultHover: l,
  reverseOnClick: c,
  markersIconCallback: A
}) {
  const u = () => {
    i(), m(), p(), B();
  }, i = () => {
    var y;
    (y = t.state.map) == null || y.on("click", (k) => {
      O(k);
    });
  }, m = () => {
    !t.state.map || (t.state.zoom = t.state.map.getView().getZoom(), t.state.map.on("moveend", () => {
      if (!t.state.map)
        return;
      const y = t.state.map.getView().getZoom();
      t.state.zoom != y && (e("on-zoom", y), t.state.zoom = y);
    }));
  }, p = () => {
    var y;
    t.actions.overlays.setupOverlays(), (y = t.state.map) == null || y.on("pointermove", function(k) {
      const b = t.actions.markers.getFeatureFromEvent(
        k
      );
      if (b) {
        const G = b.getProperties().mainMarker, D = b.get("features"), { featCoords: $, featText: x } = es(b);
        if (!D || D.length === 1) {
          o && t.actions.overlays.changeOverlayStats({
            text: Array.isArray(x) ? x[0] : x,
            coords: $,
            offset: G ? Q.high : Q.short
          });
          return;
        }
        r && r({
          changeOverlayStats: t.actions.overlays.changeOverlayStats,
          map: t.state.map,
          feature: b
        });
      }
      t.actions.overlays.changeOverlayStats();
    });
  }, B = () => {
    window.addEventListener(
      "resize",
      () => t.actions.dimensions.updateMapDimensions()
    ), window.addEventListener(
      "resize",
      t.actions.dimensions.updateBreakpoints
    );
  }, O = async (y) => {
    var x;
    let k, b;
    const G = me(y.coordinate), D = t.actions.markers.getFeatureFromEvent(y);
    if (!(D == null ? void 0 : D.getProperties().mainMarker) && t.state.mainMarker && ((x = t.state.map) == null || x.removeLayer(t.state.mainMarker)), t.actions.overlays.changeOverlayStats(void 0, "persistent"), D)
      s && V(D);
    else if (c) {
      const U = await t.actions.markers.reverseOnPoint(G, {
        markersIconCallback: A
      });
      k = U.marker, b = U.data;
    }
    e("on-click", {
      event: y,
      marker: k,
      coords: G,
      apiData: b,
      map: t.state.map,
      selectedFeature: D
    });
  }, V = (y) => {
    t.actions.overlays.changeOverlayStats();
    const k = y.get("features");
    k && k.length > 1 ? t.actions.markers.zoomToCluster(y) : t.actions.markers.selectFeauture(y, { delay: 0 });
  };
  return {
    setupMapEvents: u,
    setupClickEvent: i,
    setupZoomEvent: m,
    setupMarkerHoverEvent: p,
    handleClickEvent: O,
    handleResultHover: (y) => {
      const k = t.actions.markers.getClusterByCoords(
        y.mapCoords
      ), b = k.cluster, G = k.feature || t.actions.markers.getMarkerByCoords(y.mapCoords);
      if (!G)
        return;
      const D = we(b || G);
      l && t.actions.overlays.changeOverlayStats({
        coords: D,
        text: y.title
      }), e("on-result-hover", { item: y, cluster: b, marker: G });
    },
    handleResultClick: (y) => {
      t.actions.overlays.changeOverlayStats(), t.state.drawerActivation = false, a && t.actions.markers.selectFeauture(y), e("on-result-click", y);
    },
    getClusterExtent: Ee
  };
}
function as({
  props: e,
  containerRef: t
}) {
  const r = computed(() => e.item ? "route_type" in e.item ? e.item.route_type && Object.keys(q).includes(e.item.route_type) ? e.item.route_type : e.item.place ? "local_government_office" : "primary" : e.item.type && Object.keys(q).includes(e.item.type) ? e.item.type : e.item.category === "place" ? "local_government_office" : "primary" : void 0), s = computed(() => {
    if (!e.item)
      return;
    const o = "route_type" in e.item, l = o ? e.item.city || e.item.village ? e.item.state ? (e.item.city || e.item.village) + ", " + e.item.state : e.item.city || e.item.village : e.item.state : e.item.region;
    return {
      place: o ? e.item.place : e.item.title,
      address: o ? e.item.formatted_address : e.item.address,
      region: l,
      neighbourhood: o ? "محله " + e.item.neighbourhood : e.item.neighbourhood,
      in_odd_even_zone: o ? e.item.in_odd_even_zone : null,
      in_traffic_zone: o ? e.item.in_traffic_zone : null
    };
  }), { width: a } = rs({ containerRef: t });
  return {
    iconName: r,
    width: a,
    formattedItem: s
  };
}
function rs({
  containerRef: e
}) {
  const t = ref(0);
  watch(
    () => {
      var s;
      return (s = e.value) == null ? void 0 : s.clientWidth;
    },
    (s) => {
      s && (t.value = s);
    }
  );
  const r = () => {
    var a;
    const s = (a = e.value) == null ? void 0 : a.clientWidth;
    s && (t.value = s);
  };
  return onUpdated(() => {
    nextTick(() => r());
  }), {
    width: t
  };
}
var os = ["src", "width"];
var ns = { class: "px-2 text-lg" };
var is = {
  key: 0,
  class: "d-flex align-center"
};
var ls = createBaseVNode("br", null, null, -1);
var cs = {
  key: 1,
  class: "d-flex align-center"
};
var As = createBaseVNode("br", null, null, -1);
var us = {
  key: 2,
  class: "d-flex align-center"
};
var ds = createBaseVNode("br", null, null, -1);
var gs = {
  key: 3,
  class: "d-flex align-center"
};
var ms = createBaseVNode("br", null, null, -1);
var ws = {
  key: 4,
  class: "d-flex align-center"
};
var ys = createBaseVNode("br", null, null, -1);
var Cs = {
  key: 5,
  class: "d-flex align-center"
};
var Ie = defineComponent({
  __name: "PointDetails",
  props: {
    item: Object,
    collapse: Boolean
  },
  setup(e) {
    const t = e, r = ref(), { iconName: s, width: a, formattedItem: o } = as({
      props: t,
      containerRef: r
    });
    return (l, c) => {
      var A, u, i, m, p, B, O;
      return openBlock(), createElementBlock("div", {
        ref_key: "pointDetailsContainer",
        ref: r
      }, [
        e.item && "type" in e.item && e.item.iconUrl && !e.item.iconUrl.endsWith("general.png") ? (openBlock(), createElementBlock("img", {
          key: 0,
          src: (A = e.item) == null ? void 0 : A.iconUrl.replace(unref(W).search, unref(W).colorSvg).replace(".png", ".svg"),
          width: unref(a)
        }, null, 8, os)) : (openBlock(), createBlock(T, {
          key: 1,
          name: unref(s),
          size: unref(a),
          color: "steelblue",
          scale: false
        }, null, 8, ["name", "size"])),
        createBaseVNode("div", ns, [
          (u = unref(o)) != null && u.place ? (openBlock(), createElementBlock("div", is, [
            createVNode(T, {
              name: "marker",
              size: 25,
              color: "steelblue",
              class: "px-1"
            }),
            createTextVNode(" " + toDisplayString(unref(o).place), 1)
          ])) : createCommentVNode("", true),
          ls,
          (i = unref(o)) != null && i.address ? (openBlock(), createElementBlock("div", cs, [
            createVNode(T, {
              name: "secondary",
              size: 25,
              color: "steelblue",
              class: "px-1"
            }),
            createTextVNode(" " + toDisplayString(unref(o).address), 1)
          ])) : createCommentVNode("", true),
          As,
          (m = unref(o)) != null && m.neighbourhood ? (openBlock(), createElementBlock("div", us, [
            createVNode(T, {
              name: "neighborhood_landmark",
              size: 25,
              color: "steelblue",
              class: "px-1"
            }),
            createTextVNode(" " + toDisplayString(unref(o).neighbourhood), 1)
          ])) : createCommentVNode("", true),
          ds,
          (p = unref(o)) != null && p.region ? (openBlock(), createElementBlock("div", gs, [
            createVNode(T, {
              name: "city",
              size: 25,
              color: "steelblue",
              class: "px-1"
            }),
            createTextVNode(" " + toDisplayString(unref(o).region), 1)
          ])) : createCommentVNode("", true),
          ms,
          (B = unref(o)) != null && B.in_odd_even_zone ? (openBlock(), createElementBlock("div", ws, toDisplayString(unref(o).in_odd_even_zone ? "داخل" : "خارج از") + " محدوده طرح زوج و فرد ", 1)) : createCommentVNode("", true),
          ys,
          (O = unref(o)) != null && O.in_traffic_zone ? (openBlock(), createElementBlock("div", Cs, toDisplayString(unref(o).in_traffic_zone ? "داخل" : "خارج از") + " محدوده طرح ترافیک ", 1)) : createCommentVNode("", true)
        ])
      ], 512);
    };
  }
});
var ps = defineComponent({
  __name: "ResultsSection",
  props: {
    resultBoxClass: Array,
    resultBoxStyle: Object,
    items: {
      type: Array,
      default: () => []
    }
  },
  emits: ["result-hover", "result-click"],
  setup(e, { emit: t }) {
    const r = e, s = inject("store"), a = ref(), o = (A) => {
      s.getters.touchPlatform || l(A);
    }, l = (A) => {
      t("result-hover", A);
    }, c = (A) => {
      t("result-click", A);
    };
    return watch(
      () => s.state.drawerActivation,
      (A) => {
        var i;
        if (!A)
          return;
        const u = s.getters.touchPlatform ? `height: ${s.state.mapDimensions.height}; width: ${s.getters.drawerWidth}; ${r.resultBoxStyle}` : `height: ${s.state.mapDimensions.height}; width: ${s.getters.drawerWidth}; ${r.resultBoxStyle}`;
        (i = a.value) == null || i.setAttribute("style", u);
      }
    ), (A, u) => (openBlock(), createElementBlock("div", {
      style: normalizeStyle(e.resultBoxStyle),
      class: normalizeClass([e.resultBoxClass, "map-result-box mx-auto"]),
      ref_key: "resultSection",
      ref: a
    }, [
      (unref(s).getters.touchPlatform ? unref(s).state.searchLoading : unref(s).getters.loading) ? (openBlock(), createBlock(ke, { key: 0 })) : createCommentVNode("", true),
      !unref(s).getters.touchPlatform && unref(s).state.drawerShowDetails && unref(s).state.selectedMarker ? (openBlock(), createBlock(Ie, {
        key: 1,
        item: unref(s).state.selectedMarker
      }, null, 8, ["item"])) : (openBlock(true), createElementBlock(Fragment, { key: 2 }, renderList(e.items, (i) => (openBlock(), createBlock(Ot, {
        key: "" + i.location.x + i.location.y,
        item: i,
        onMouseenter: (m) => o(i),
        onClick: (m) => c(i)
      }, null, 8, ["item", "onMouseenter", "onClick"]))), 128))
    ], 6));
  }
});
var hs = j(ps, [["__scopeId", "data-v-2d187be2"]]);
var vs = ["activated"];
var Ms = defineComponent({
  __name: "DrawerComp",
  props: {
    items: Array,
    loading: Boolean
  },
  emits: ["search", "result-click", "result-hover"],
  setup(e, { emit: t }) {
    const r = inject("store"), s = ref(""), a = (i) => {
      const m = {
        term: i.term || ""
      };
      t("search", m);
    }, o = (i) => {
      t("result-click", i);
    }, l = (i) => {
      t("result-hover", i);
    }, c = ref(), A = ref(), u = (i) => {
      if (!(!A.value || !c.value))
        if (i)
          c.value.style.setProperty("max-height", "100%"), c.value.style.setProperty("max-width", "100%");
        else {
          const m = A.value.$el.clientHeight, p = A.value.$el.clientWidth;
          c.value.style.setProperty("max-height", `${m}px`), c.value.style.setProperty("max-width", `${p}px`);
        }
    };
    return watch(
      () => r.state.drawerActivation,
      (i) => {
        u(i);
      }
    ), onMounted(() => {
      u(r.state.drawerActivation);
    }), (i, m) => (openBlock(), createElementBlock("div", {
      class: normalizeClass([{
        "map-drawer--mobile": unref(r).getters.touchPlatform,
        "map-drawer--desktop": !unref(r).getters.touchPlatform,
        "map-drawer--desktop---activated": !unref(r).getters.touchPlatform && unref(r).state.drawerActivation,
        "box-shadow": !unref(r).state.drawerActivation
      }, "d-flex flex-column"]),
      activated: unref(r).state.drawerActivation,
      ref_key: "mapDrawer",
      ref: c
    }, [
      createVNode(bt, {
        ref_key: "searchSection",
        ref: A,
        searchText: s.value,
        "onUpdate:searchText": m[0] || (m[0] = (p) => s.value = p),
        onSubmit: m[1] || (m[1] = (p) => a({ term: p }))
      }, null, 8, ["searchText"]),
      createVNode(hs, {
        items: e.items,
        onResultClick: o,
        onResultHover: l
      }, null, 8, ["items"])
    ], 10, vs));
  }
});
var ks = j(Ms, [["__scopeId", "data-v-a45799c9"]]);
var Ds = (e) => (pushScopeId("data-v-c083d67c"), e = e(), popScopeId(), e);
var fs = Ds(() => createBaseVNode("span", { class: "d-block" }, null, -1));
var Bs = [
  fs
];
var bs = defineComponent({
  __name: "MobileDetailsSection",
  setup(e) {
    const t = inject("store"), r = ref(), s = ref(), a = ref(false), o = ref(R.bottomSheetNormalHeight), l = ref(
      R.bottomSheetBorderRadius
    ), c = ref("0"), A = (M) => {
      if (M.preventDefault(), !r.value || !t.state.mapContainer)
        return;
      const y = t.state.mapContainer.getBoundingClientRect().bottom - M.touches[0].clientY;
      o.value = y + "px";
    }, u = () => {
      if (!s.value)
        return;
      p();
      const M = o.value, Y = Number(M.slice(0, M.length - 2)), y = Number(
        t.state.mapDimensions.height.slice(
          0,
          t.state.mapDimensions.height.length - 2
        )
      );
      a.value ? Y < y * 0.8 ? i() : o.value = t.state.mapDimensions.height : Y > y / 4 ? (o.value = `calc(${t.state.mapDimensions.height})`, l.value = "0", a.value = true) : o.value = R.bottomSheetNormalHeight;
    }, i = () => {
      a.value = false, p(), l.value = R.bottomSheetBorderRadius, o.value = R.bottomSheetNormalHeight;
    };
    let m;
    const p = (M = 0.5) => {
      clearTimeout(m), c.value = M + "s", m = window.setTimeout(() => {
        c.value = "none";
      }, M * 1e3);
    }, B = () => {
      t.actions.markers.deselectAll();
    }, O = () => {
      o.value = R.bottomSheetloadingHeight;
    }, V = () => {
      o.value = R.bottomSheetNormalHeight;
    };
    return watch(
      () => t.state.reverseLoading,
      (M) => {
        p(0.2), M ? O() : a.value || V();
      }
    ), watch(
      () => t.state.selectedMarker,
      (M) => {
        !M || (p(0.4), O(), setTimeout(() => {
          V();
        }, 200));
      }
    ), (M, Y) => (openBlock(), createElementBlock("div", {
      ref_key: "mobileDetailsSectionContainer",
      ref: r,
      class: normalizeClass(["mobile-details-section pos-absolute", { fullScreen: a.value }])
    }, [
      createBaseVNode("button", {
        class: "close-modal-button pos-absolute d-flex justify-center align-center",
        onClick: B
      }, [
        createVNode(T, { size: 25 })
      ]),
      createBaseVNode("div", {
        ref_key: "detailsContainer",
        ref: s,
        class: normalizeClass(a.value ? "o-auto" : "o-hidden"),
        style: normalizeStyle(`
        max-height: ${o.value};
        border-radius: ${l.value};
        transition: ${c.value};`)
      }, [
        withDirectives(createVNode(ke, { class: "curved-loading" }, null, 512), [
          [vShow, unref(t).state.reverseLoading]
        ]),
        createVNode(T, {
          name: "close",
          size: 40,
          style: normalizeStyle(`visibility: ${a.value ? "visible" : "hidden"};`),
          class: "close-icon pos-absolute",
          onClick: i
        }, null, 8, ["style"]),
        createBaseVNode("span", {
          class: "drag-button d-block pos-absolute",
          style: normalizeStyle(`padding-top: calc(${unref(R).bottomSheetloadingHeight}/2)`),
          onTouchmove: A,
          onTouchend: u
        }, Bs, 36),
        createBaseVNode("div", {
          style: normalizeStyle(`
      height: ${unref(t).state.mapDimensions.height}px;
      margin-top: ${unref(R).bottomSheetloadingHeight};`)
        }, [
          createVNode(Ie, {
            item: unref(t).state.selectedMarker
          }, null, 8, ["item"])
        ], 4)
      ], 6)
    ], 2));
  }
});
var Ls = j(bs, [["__scopeId", "data-v-c083d67c"]]);
var Es = (e) => ({
  "Api-Key": e
});
var Hs = (e, t) => {
  const r = {
    headers: Es(e)
  }, s = t != null && t.reverseUrl ? {} : r, a = t != null && t.searchUrl ? {} : r;
  return {
    REVERSE: async (o, l) => {
      const c = `?lat=${l}&lng=${o}`, A = ((t == null ? void 0 : t.reverseUrl) || le.reverse) + c;
      return await (await fetch(A, s)).json();
    },
    SEARCH: async (o, [l, c]) => {
      const A = `?term=${o}&lat=${c}&lng=${l}`, u = ((t == null ? void 0 : t.searchUrl) || le.search) + A;
      return await (await fetch(u, a)).json();
    }
  };
};
var Is = () => ({
  breakpoints: {
    ...se,
    lt: { ...se },
    gt: { ...se }
  },
  mapDimensions: Dt
});
var Rs = Is();
var Ss = () => ({
  selectedMarker: null,
  mainMarker: null,
  mainMarkerCoords: null,
  reverseResult: null,
  searchMarkers: null,
  searchResults: []
});
var Os = Ss();
var Gs = () => ({
  overlay: null,
  persistentOverlay: null,
  popupContainer: null,
  persistentContainer: null
});
var zs = Gs();
var Ts = () => ({
  drawerActivation: false,
  drawerShowDetails: false,
  mobileDrawerShowDetails: false
});
var Ps = Ts();
var Ys = () => ({
  mapContainer: null,
  map: null,
  mapType: "neshan",
  zoom: ae.initialZoom,
  poiLayer: true,
  trafficLayer: true,
  viewType: "desktop",
  scale: 1
});
var js = Ys();
var xs = {
  ...Rs,
  ...Os,
  ...zs,
  ...Ps,
  ...js,
  api: null,
  searchLoading: false,
  reverseLoading: false
};
var Qs = () => reactive(JSON.parse(JSON.stringify(xs)));
var Vs = (e) => reactive({
  drawerWidth: computed(() => (e.viewType ? e.viewType === "mobile" : pe()) ? e.mapDimensions.width : R.width),
  loading: computed(() => e.searchLoading || e.reverseLoading),
  touchPlatform: computed(() => e.viewType ? e.viewType === "mobile" : pe())
});
var Fs = ({ state: e }, t, r) => {
  var o;
  const { layer: s, style: a } = Qt(t, r);
  return (o = e.map) == null || o.addLayer(s), { layer: s, style: a };
};
var Js = (e, t, r) => {
  if (!t)
    return;
  const s = t.getSource().getFeatures() || [];
  if (!s.some((c) => c.get("isCluster")))
    return;
  const a = s.reduce((c, A) => {
    const u = A.get("features");
    return [...c, ...u];
  }, []), o = r ? 0 : 30, l = Le(a, {
    distance: o,
    minDistance: o
  });
  t.setSource(l);
};
var Ns = (e, t) => {
  var o;
  const r = (o = e.state.searchMarkers) == null ? void 0 : o.getSource().getFeatures();
  let s;
  const a = r == null ? void 0 : r.find((l) => {
    const c = Re(e, l, t);
    return c && (s = c), c;
  });
  return {
    feature: s,
    cluster: a
  };
};
var Re = (e, t, r) => {
  const s = t.get("features");
  return s == null ? void 0 : s.find((a) => a.getId() === r.join("-"));
};
var Zs = ({ state: e }, t) => {
  var r;
  return (r = e.searchMarkers) == null ? void 0 : r.getSource().getFeatureById(t.join("-"));
};
var Ws = ({ state: e }, t) => {
  const r = t.get("text");
  return e.searchResults.find(
    (s) => s.title === (Array.isArray(r) ? r[0] : r)
  );
};
var Xs = (e, t, r) => {
  const s = t.getSource().getExtent();
  _(e, s, r);
};
var _ = ({ state: e, getters: t }, r, s) => {
  var o;
  const a = (s == null ? void 0 : s.duration) || 1e3;
  (o = e.map) == null || o.getView().fit(r, {
    size: e.map.getSize(),
    duration: a,
    maxZoom: e.zoom > ae.maxZoom ? e.zoom : ae.maxZoom,
    padding: [15, t.touchPlatform ? 15 : 300, 15, 15]
  });
};
var Se = (e, t, r) => {
  const s = He(t);
  _(e, s, r);
};
var Ks = (e, t, r) => {
  const s = Ee(t);
  _(e, s, r);
};
var Oe = (e, t, r) => {
  const s = [...t, ...t];
  _(e, s, r);
};
var Us = (e, t, r) => {
  let s = false, a = r == null ? void 0 : r.coords, o = r == null ? void 0 : r.text;
  "mapCoords" in t ? (a || (a = t.mapCoords), Oe(e, a), "title" in t ? o = t.title : (s = true, o = De(t))) : (Se(e, t), a || (a = we(t)), s = t == null ? void 0 : t.getProperties().mainMarker, o || (o = t.getProperties().text), Array.isArray(o) && (o = o[0])), (r == null ? void 0 : r.delay) !== 0 ? setTimeout(() => {
    e.actions.overlays.changeOverlayStats(
      e,
      {
        coords: a,
        text: o,
        offset: s ? Q.high : Q.short
      },
      "persistent"
    );
  }, (r == null ? void 0 : r.delay) || 1e3) : e.actions.overlays.changeOverlayStats(
    e,
    {
      coords: a,
      text: o,
      offset: s ? Q.high : Q.short
    },
    "persistent"
  );
  const l = s ? e.state.reverseResult : "mapCoords" in t ? t : e.actions.markers.getSearchResultByFeature(e, t);
  l && (e.state.selectedMarker = l, e.actions.drawers.toggleResultDrawers(e, true));
};
var qs = (e) => {
  var t;
  e.actions.overlays.changeOverlayStats(e, void 0, "persistent"), e.state.mainMarker && ((t = e.state.map) == null || t.removeLayer(e.state.mainMarker)), e.state.selectedMarker = null, e.actions.drawers.toggleResultDrawers(e, false);
};
var _s = (e, t) => {
  var r;
  return (r = e.state.map) == null ? void 0 : r.forEachFeatureAtPixel(t.pixel, (s) => s);
};
var $s = async (e, t, {
  useMarker: r = true,
  usePopup: s = true,
  customText: a,
  markersIconCallback: o
} = {}) => {
  var l;
  try {
    e.state.reverseLoading = true;
    const c = me(t, "EPSG:4326", "EPSG:3857");
    e.getters.touchPlatform ? e.state.mobileDrawerShowDetails = true : e.state.drawerActivation = true;
    let A;
    if (r) {
      const { layer: p } = e.actions.markers.addMarkers(
        e,
        [
          {
            coords: c,
            text: "",
            iconScale: 0.1,
            iconUrl: W.main,
            isReverseMarker: true
          }
        ],
        {
          markersIconCallback: o,
          props: {
            isCluster: false
          },
          anchor: [0.5, 1]
        }
      );
      e.state.mainMarkerCoords = t, e.state.mainMarker = p, A = p;
    }
    if (!e.state.api)
      throw "No reverse api";
    const u = await e.state.api.REVERSE(...t), i = {
      ...u,
      mapCoords: t
    };
    e.state.selectedMarker = i, e.state.reverseResult = i, e.actions.drawers.toggleResultDrawers(e, true);
    const m = a || De(i);
    return (l = e.state.mainMarker) == null || l.getSource().getFeatures()[0].set("text", m), s && e.actions.overlays.changeOverlayStats(
      e,
      { coords: c, text: m, offset: Q.high },
      "persistent"
    ), { marker: A, coords: t, data: u };
  } catch (c) {
    return console.log(c), {};
  } finally {
    e.state.reverseLoading = false;
  }
};
var ea = async (e, { term: t = "", coords: r }, s) => {
  var a;
  try {
    if (e.state.searchLoading = true, !e.state.api)
      return;
    const o = await e.state.api.SEARCH(t, r);
    e.state.drawerShowDetails = false, e.state.searchMarkers && ((a = e.state.map) == null || a.removeLayer(e.state.searchMarkers));
    const l = await Nt(o.items), c = l.map((u) => ({
      ...u.originalItem,
      iconUrl: u.iconUrl,
      mapCoords: u.coords
    }));
    e.state.searchResults = c;
    const { layer: A } = e.actions.markers.addMarkers(e, l, {
      markersIconCallback: s == null ? void 0 : s.markersIconCallback,
      cluster: s == null ? void 0 : s.cluster,
      clusterThreshold: s == null ? void 0 : s.clusterThreshold
    });
    return e.actions.overlays.changeOverlayStats(
      e,
      void 0,
      "persistent"
    ), e.state.searchMarkers = A, setTimeout(() => {
      const u = A.getSource().getFeatures();
      u.length === 1 ? e.actions.markers.zoomToCluster(e, u[0], {
        duration: 1500
      }) : e.actions.markers.zoomToLayer(e, A, { duration: 1500 });
    }, 200), {
      markers: A,
      data: o
    };
  } catch (o) {
    console.log(o);
  } finally {
    e.state.searchLoading = false;
  }
};
var ta = {
  addMarkers: Fs,
  toggleClusterSource: Js,
  getClusterByCoords: Ns,
  getMarkerInClusterByCoords: Re,
  getMarkerByCoords: Zs,
  getSearchResultByFeature: Ws,
  zoomToLayer: Xs,
  zoomToExtent: _,
  zoomToCoords: Oe,
  zoomToMarker: Se,
  zoomToCluster: Ks,
  selectFeauture: Us,
  deselectAll: qs,
  getFeatureFromEvent: _s,
  reverseOnPoint: $s,
  search: ea
};
var sa = (e) => {
  !e.state.mapContainer || (e.state.mapDimensions = {
    height: e.state.mapContainer.clientHeight + "px",
    width: e.state.mapContainer.clientWidth + "px"
  });
};
var aa = (e) => {
  const t = window.innerWidth, r = Object.keys(se), s = JSON.parse(
    JSON.stringify(toRaw(e.state.breakpoints))
  );
  r.forEach((a, o) => {
    const l = r[o + 1];
    t >= ie[a] ? (s.lt[a] = false, !l || t < ie[l] ? (s[a] = true, s.gt[a] = false) : (s[a] = false, s.gt[a] = true)) : t < ie[a] && (s.lt[a] = true, s.gt[a] = false);
  }), e.state.breakpoints = s;
};
var ra = {
  updateMapDimensions: sa,
  updateBreakpoints: aa
};
var oa = (e) => {
  var s, a;
  if (!e.state.popupContainer || !e.state.persistentContainer)
    return;
  const t = ce(e, e.state.popupContainer);
  e.state.overlay = t;
  const r = ce(
    e,
    e.state.persistentContainer,
    true
  );
  e.state.persistentOverlay = r, (s = e.state.map) == null || s.addOverlay(t), (a = e.state.map) == null || a.addOverlay(r);
};
var na = (e, t, r = "temporary") => {
  const s = r === "temporary" ? e.state.popupContainer : e.state.persistentContainer, a = r === "temporary" ? e.state.overlay : e.state.persistentOverlay;
  if (!!s)
    if (t) {
      const { coords: o, text: l, offset: c } = t;
      s.innerHTML = l, c && (a == null || a.setOffset(c)), a == null || a.setPosition(o);
    } else
      a == null || a.setPosition(void 0);
};
var ce = (e, t, r = false) => {
  const s = new ol.Overlay({
    element: t,
    map: e.state.map,
    positioning: "top-center",
    offset: Q.short
  });
  return s.set("persistent", r), s;
};
var ia = {
  setupOverlays: oa,
  createOverlay: ce,
  changeOverlayStats: na
};
var la = (e, t = !e.state.drawerShowDetails) => {
  e.getters.touchPlatform ? e.state.mobileDrawerShowDetails = t : (e.state.drawerShowDetails = t, e.state.drawerActivation = t);
};
var ca = {
  toggleResultDrawers: la
};
var Aa = (e, t = 0) => {
  setTimeout(() => {
    var r;
    return (r = e.state.map) == null ? void 0 : r.updateSize();
  }, t);
};
var ua = (e, t = !e.state.poiLayer) => {
  var r;
  e.state.poiLayer = t, (r = e.state.map) == null || r.switchPoiLayer(t);
};
var da = (e, t = !e.state.trafficLayer) => {
  var r;
  e.state.trafficLayer = t, (r = e.state.map) == null || r.switchTrafficLayer(t);
};
var ga = (e, t) => {
  var r;
  e.state.mapType = t, (r = e.state.map) == null || r.setMapType(t);
};
var ma = {
  shakeMap: Aa,
  togglePoi: ua,
  toggleTraffic: da,
  setMapType: ga
};
var te = {
  markers: ta,
  dimensions: ra,
  overlays: ia,
  drawers: ca,
  map: ma
};
var wa = (e, t) => (...r) => e(t, ...r);
var ya = () => {
  const e = Qs(), t = Vs(e), r = {
    actions: te,
    getters: t,
    state: e
  }, a = Object.keys(te).reduce((o, l) => {
    const A = Object.keys(te[l]).reduce((u, i) => ({
      ...u,
      [i]: wa(
        te[l][i],
        r
      )
    }), {});
    return { ...o, [l]: A };
  }, {});
  return { state: e, getters: t, actions: a };
};
var Ca = (e) => (pushScopeId("data-v-318b10ed"), e = e(), popScopeId(), e);
var pa = Ca(() => createBaseVNode("img", { src: Fe }, null, -1));
var ha = [
  pa
];
var va = {
  name: "NeshanMap"
};
var Ma = defineComponent({
  ...va,
  props: {
    mapKey: {
      type: String,
      required: true
    },
    serviceKey: {
      type: String,
      default: ""
    },
    reverseUrl: String,
    searchUrl: String,
    center: {
      type: Object,
      default: null
    },
    zoom: {
      type: Number,
      default: ae.initialZoom
    },
    poi: {
      type: Boolean,
      default: true
    },
    traffic: {
      type: Boolean,
      default: true
    },
    defaultType: {
      type: String,
      default: "neshan"
    },
    mapTypes: {
      type: Array,
      default: Ce.map((e) => e.title)
    },
    desktopLayersClass: [String, Array, Object],
    mobileLayersClass: [String, Array, Object],
    desktopLayersStyle: Object,
    mobileLayersStyle: Object,
    temporaryPopupContainerClass: [String, Array, Object],
    persistentPopupContainerClass: [String, Array, Object],
    temporaryPopupContainerStyle: Object,
    persistentPopupContainerStyle: Object,
    hideLayers: Boolean,
    hideSearchContainer: Boolean,
    markersIconCallback: Function,
    markerHoverCallback: Function,
    popupOnMarkerHover: {
      type: Boolean,
      default: true
    },
    popupOnResultHover: {
      type: Boolean,
      default: true
    },
    zoomOnMarkerClick: {
      type: Boolean,
      default: true
    },
    zoomOnResultClick: {
      type: Boolean,
      default: true
    },
    cluster: {
      type: Boolean,
      default: true
    },
    clusterThreshold: {
      type: Number,
      default: 18
    },
    reverseOnClick: {
      type: Boolean,
      default: true
    },
    scale: {
      type: Number,
      default: 1
    },
    viewType: String
  },
  emits: [
    "on-zoom",
    "on-click",
    "on-result-hover",
    "on-result-click"
  ],
  setup(e, { expose: t, emit: r }) {
    const s = e, a = ya();
    provide("store", a);
    const o = (d, h) => {
      a.state.api = Hs(d, h);
    };
    o(s.serviceKey, {
      searchUrl: s.searchUrl,
      reverseUrl: s.reverseUrl
    }), watch(
      () => [s.serviceKey, s.searchUrl, s.reverseUrl],
      (d) => {
        o(d[0], {
          searchUrl: d[1],
          reverseUrl: d[2]
        });
      }
    );
    const l = ref(s.scale + "rem");
    a.state.scale = s.scale, watch(
      () => s.scale,
      (d) => {
        l.value = d + "rem", a.state.scale = d, u();
      }
    );
    let c;
    const A = () => {
      var h, z;
      if (c)
        return c;
      const d = (z = (h = B.value) == null ? void 0 : h.querySelector(".ol-viewport")) == null ? void 0 : z.querySelectorAll("div:last-child");
      if (!!d)
        return c = Array.from(d)[d.length - 1], c;
    }, u = () => {
      const d = A();
      d == null || d.style.setProperty("scale", "" + s.scale);
    }, i = () => {
      const d = A();
      d == null || d.style.setProperty("bottom", "0.2em"), d == null || d.style.setProperty("left", "0.4em");
    };
    a.state.viewType = s.viewType, watch(
      () => s.viewType,
      (d) => {
        a.state.viewType = d;
      }
    );
    const m = ref(false), p = reactive(
      Ce.filter((d) => s.mapTypes.includes(d.title))
    );
    a.actions.map.toggleTraffic(s.traffic), a.actions.map.togglePoi(s.poi), watch(
      () => s.traffic,
      (d) => {
        a.actions.map.toggleTraffic(d);
      }
    ), watch(
      () => s.poi,
      (d) => {
        a.actions.map.togglePoi(d);
      }
    );
    const B = ref(), O = (d, h = "my-openlayer") => {
      const z = document.getElementById(h);
      if (z)
        return z;
      const Z = document.createElement("script");
      return Z.src = d, Z.id = h, document.getElementsByTagName("head")[0].appendChild(Z), Z;
    }, V = ref(fe(s.center)), M = async () => {
      if (!B.value)
        return;
      const d = V.value || await jt(), h = new ol.Map({
        target: B.value,
        key: s.mapKey,
        poi: a.state.poiLayer,
        traffic: a.state.trafficLayer,
        view: new ol.View({
          center: ol.proj.fromLonLat(d),
          zoom: s.zoom,
          smoothExtentConstraint: true
        }),
        controls: []
      });
      a.state.map = h, a.state.map.setMapType(s.defaultType), a.state.mapType = s.defaultType, a.actions.map.shakeMap(300);
    }, Y = ref(), y = ref(), { setupMapEvents: k, handleResultHover: b, handleResultClick: G } = ss({
      emits: r,
      store: a,
      markerHoverCallback: s.markerHoverCallback,
      markersIconCallback: s.markersIconCallback,
      zoomOnMarkerClick: s.zoomOnMarkerClick,
      zoomOnResultClick: s.zoomOnResultClick,
      popupOnMarkerHover: s.popupOnMarkerHover,
      popupOnResultHover: s.popupOnResultHover,
      reverseOnClick: s.reverseOnClick
    });
    watch(
      () => a.state.zoom,
      (d, h) => {
        !a.state.searchMarkers || (d >= s.clusterThreshold && h < s.clusterThreshold ? a.actions.markers.toggleClusterSource(a.state.searchMarkers, true) : d < s.clusterThreshold && h >= s.clusterThreshold && a.actions.markers.toggleClusterSource(
          a.state.searchMarkers,
          false
        ));
      }
    );
    const D = async ({ term: d, coords: h }) => {
      const z = h || a.state.mainMarkerCoords || V.value;
      if (!z || !a.state.api)
        return;
      const Z = {
        cluster: s.cluster,
        clusterThreshold: s.clusterThreshold,
        markersIconCallback: s.markersIconCallback
      };
      return await a.actions.markers.search({ term: d, coords: z }, Z);
    }, $ = async ({ term: d, coords: h }) => (a.state.drawerActivation = true, await D({ term: d, coords: h })), x = (d) => {
      const h = d.composedPath()[1];
      h.classList.add("floaten"), setTimeout(() => h.classList.remove("floaten"), 350), m.value = true;
    };
    t({
      state: a.state,
      search: $,
      reverse: a.actions.markers.reverseOnPoint
    });
    const U = async () => {
      await M(), k(), a.actions.dimensions.updateMapDimensions(), u(), i();
    };
    return a.actions.dimensions.updateBreakpoints(), onMounted(() => {
      B.value && (a.state.mapContainer = B.value), Y.value && (a.state.popupContainer = Y.value), y.value && (a.state.persistentContainer = y.value), typeof ol < "u" ? U() : O(le.map).addEventListener("load", () => {
        U();
      });
    }), (d, h) => (openBlock(), createElementBlock("div", {
      ref_key: "mapContainer",
      ref: B,
      class: normalizeClass(["map pos-relative o-hidden", {
        touch: unref(a).getters.touchPlatform
      }]),
      style: normalizeStyle(`font-size: ${l.value}`)
    }, [
      createBaseVNode("div", {
        ref_key: "popupContainer",
        ref: Y,
        class: normalizeClass(["map-popup-container", e.temporaryPopupContainerClass]),
        style: normalizeStyle(e.temporaryPopupContainerStyle)
      }, null, 6),
      createBaseVNode("div", {
        ref_key: "persistentContainer",
        ref: y,
        class: normalizeClass(["map-popup-container", e.persistentPopupContainerClass]),
        style: normalizeStyle(e.persistentPopupContainerStyle)
      }, null, 6),
      e.hideLayers ? createCommentVNode("", true) : renderSlot(d.$slots, "layers", {
        key: 0,
        tiles: p,
        mapType: unref(a).state.mapType,
        poi: unref(a).state.poiLayer,
        traffic: unref(a).state.trafficLayer
      }, () => [
        unref(a).getters.touchPlatform ? (openBlock(), createBlock(vt, {
          key: 1,
          modelValue: m.value,
          "onUpdate:modelValue": h[0] || (h[0] = (z) => m.value = z),
          tiles: p,
          layersClass: e.mobileLayersClass,
          layersStyle: e.mobileLayersStyle
        }, null, 8, ["modelValue", "tiles", "layersClass", "layersStyle"])) : (openBlock(), createBlock(rt, {
          key: 0,
          tiles: p,
          layersClass: e.desktopLayersClass,
          layersStyle: e.desktopLayersStyle
        }, null, 8, ["tiles", "layersClass", "layersStyle"])),
        unref(a).getters.touchPlatform ? (openBlock(), createElementBlock("button", {
          key: 2,
          class: "mobile-layers-button pos-absolute justify-center align-center d-flex pointer",
          onClick: x
        }, ha)) : createCommentVNode("", true)
      ], true),
      e.hideSearchContainer ? createCommentVNode("", true) : renderSlot(d.$slots, "search-container", { key: 1 }, () => [
        createVNode(ks, {
          items: unref(a).state.searchResults,
          onSearch: D,
          onResultClick: unref(G),
          onResultHover: unref(b)
        }, null, 8, ["items", "onResultClick", "onResultHover"]),
        unref(a).getters.touchPlatform ? withDirectives((openBlock(), createBlock(Ls, { key: 0 }, null, 512)), [
          [vShow, unref(a).state.mobileDrawerShowDetails]
        ]) : createCommentVNode("", true)
      ], true)
    ], 6));
  }
});
var Da = j(Ma, [["__scopeId", "data-v-318b10ed"]]);
export {
  Da as default
};
//# sourceMappingURL=@neshan-maps-platform_vue3-openlayers.js.map
